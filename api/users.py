...
#retrieve the username and password of a user and see if it matches user inputs
@main.route('\login', methods = ['POST'])
def log_in(username, password):

    exists_username = db.session.query(db.exists().where(User.user_id == username)).scalar()
    
    if exists_username:
        right_password = db.session.query(db.exists().where(User.pass_word == password)).scalar()
